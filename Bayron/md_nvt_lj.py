...
############# Leer el run.json directamente ################
with open('run_init_rand.json','r') as file:
    nml = json.load(file)
#############
...
for blk in range(1,nblock+1): # Loop over blocks
    ...
    for stp in range(nstep): # Loop over steps
        ...
        if 0 == a%(nstep/10):
            cnf_prefix = "cnf_"+str(b)+"."
            write_cnf_atoms ( cnf_prefix+sav_tag, n, box, r*box, v )
            b+=1

    blk_end(blk)                                             # Output block averages

    #ESCRIBE Y LA GUARDA EN UN INP
